function EqTextArea(preview, input, comment, download, intro) {
  ...
  this.textchanged = function () {
    ...
    if (txt.length == 0) EqEditor.Opacity.fadein(this.intro_text); else EqEditor.Opacity.fadeout(this.intro_text);
    if (txt != this.orgtxt) {
      this.orgtxt = txt;
      this.changed = true;
      return true;
...
var EqEditor = {
  ...
  Gallery: null,
  Example: {
    lastbutton: null, load_json: function (file, text) {
  ...
  Panel: {
    plock: null, ctimer: null, otimer: null, oid: null, timer: Array(), setstate: function (a, b) {
      if (a) {
        var id = a.id;
        if (this.timer[id] != '') {
      ...
      var divElem;
      if (editorid !== undefined) divElem = EqEditor.$(editorid); else divElem = document;
      ...
      for (i = 0; i < panels.length; i++)if (panels[i].id != '') {
  ...
  Cookie: {
    nocookies: false, set: function (c_name, value, expiredays) {
  ...
  ExportButton: {
    ...
    state: function (state) {
      for (i = 0; i < this.bsize; i++) {
        if (state) this.bArray_id[i].className = 'lightbluebutton'; else this.bArray_id[i].className = 'greybutton';
      }
    },
  ...
  init: function (SID, obj, resize, editorid) {
    ...
    EqEditor.OnChange('type', function (e) {
      var type = EqEditor.$('type');
      if (type) {
        EqEditor.Cookie.set('type', type.value, 10);
      }
